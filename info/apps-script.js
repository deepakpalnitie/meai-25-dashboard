...
function doCombineKml(sh, curCelAddress) {
    if (curCelAddress.getA1Notation() != "F2") {
        console.log("Incorrect trigger cell for doCombineKml()");
        return;
    }

    curCelAddress.setValue(false); // Uncheck the trigger box
    let resetKMLRng = sh.getRange("F3");
    let resetInstruction = resetKMLRng.getValue();

    if (resetInstruction == "resetKML") {
        console.log("Reset instruction found. Starting KML reset process.");
        resetKml();
        resetKMLRng.setValue(""); // Clear the instruction
    } else {
        console.log("No reset instruction. Starting incremental KML combination.");
        combineKml();
    }
...
function resetKml() {
    ...
    if (lRow >= data_strt_rw) {
        ...
        console.log(`Reset ${lRow - data_strt_rw + 1} checkboxes in KML_CMBND column.`);
    }
...
<fill>1</fill>
    ...
    try {
        ...
        console.log("Successfully reset the combined KML file.");
    } catch (e) {
        console.error("Failed to write to combined KML file during reset: " + e.toString());
    }
}
...
const combineKml = () => {
    ...
    for (let i = 0; i < kmlCombinedValues.length; i++) {
        // Find the first uncombined row and process it
                ...
                try {
                    ...
                    if (fileId && fileId[0]) {
                        ...
                        if (placemarks) {
                            ...
                            if (insertionPoint !== -1) {
                                const newContent = combinedData.substring(0, insertionPoint) + placemarks.join('\n') + '\n</Document>\n</kml>';
                                combinedFile.setContent(newContent);
                                sh.getRange(currentRow, edtKmlCmbndCol).setValue(true);
                                console.log(`Appended KML from row ${currentRow}. Continuing...`);
                            }
                        }
                    }
                } catch (e) {
                    console.error(`Error on row ${currentRow}: ${e.toString()}`);
                    console.log("Stopping due to error. Please fix the issue and re-run.");
                    return; // Stop on error, but not on success
                }
            }
        }
    }
    console.log("KML combination run finished. If the script timed out, re-run to continue.");
};
...
const findKMLDuplicates = () => {
  ...
  const lRow = sh.getLastRow(); // More robust way to get the last row
